...
def get_acc(q,f,m):
    ...
    acc = MU.quat_apply_rot(MU.quat_conj(q),f/m) + np.random.normal(loc=0.0, scale=0.05, size=3)
...
def get_gyro(w):
    ...
    Parameters:
        w (numpy.ndarray): Body angular velocity [wx, wy, wz]
    """

    # Compute gyroscope measurement and add random noise
    gyro = w + np.random.normal(loc=0.0, scale=0.02, size=3)
...
def get_mag(q,tau):
    ...
    Parameters:
        q (numpy.ndarray): Orientation quaternion [qw, qx, qy, qz]
        tau (float): Thrust generated by the drone # TODO: add influence of the thrust
    """

    mag = np.array(earth_mag_field)

    # Compute Earth magnetic field on the body frame and add noise
    mag =  MU.quat_apply_rot(MU.quat_conj(q),mag) + np.random.normal(loc=0.0, scale=0.01, size=3)
...
def get_gps(p,vw):
    ...
    # Populate dictionary
    gps['i_lat__degE7'] = ( lat0+p[0]*meters2ged_lat + crandom()*5e-8 )*1e7    # Latitude (WGS84) [degE7] (type:int32_t)
    gps['i_lon__degE7'] = ( lon0-p[1]*meters2ged_lon + crandom()*5e-8 )*1e7    # Longitude (WGS84) [degE7] (type:int32_t)
    ...
    gps['i_vel__cm/s'] = ( sqrt(vw[0]**2+vw[1]**2)*0 + random()*0.001 )*100    # GPS ground speed. If unknown, set to: 65535 [cm/s] (type:uint16_t)
    gps['i_vn__cm/s'] = ( vw[0]*0 + random()*0.001 )*100                       # GPS velocity in north direction in earth-fixed NED frame [cm/s] (type:int16_t)
    gps['i_ve__cm/s'] = ( -vw[1]*0 + random()*0.001 )*100                      # GPS velocity in east direction in earth-fixed NED frame [cm/s] (type:int16_t)
    gps['i_vd__cm/s'] = ( vw[2]*0 + random()*0.001 )*100                       # GPS velocity in down direction in earth-fixed NED frame [cm/s] (type:int16_t)
...
def get_ground_truth(p,vw,q,omega):
    ...
    gt['i_lat__degE7'] = (lat0+p[0]*meters2ged_lat)*1e7    # Latitude [degE7] (type:int32_t)
    gt['i_lon__degE7'] = (lon0-p[1]*meters2ged_lon)*1e7    # Longitude [degE7] (type:int32_t)
    gt['i_alt__mm'] = h0 + p[2]                            # Altitude [mm] (type:int32_t)
    gt['vx'] = vw[0]*100                                   # Ground X Speed (Latitude) [cm/s] (type:int16_t)
    gt['vy'] = -vw[1]*100                                  # Ground Y Speed (Longitude) [cm/s] (type:int16_t)
